...
import { hasExportFlag, getClassInfo, convertClassToUI5Extend, isSAPUIDefineCallExpression } from './helpers/classes'
import { getPropNames, collapseNamedExports } from './helpers/exports'
// import { setInteroptFlag } from './import-helper'
...
module.exports = ({ types: t }) => {
      ...
      enter(path) {
        ...
        if (doesClassExtendFromImport(node, [...this.imports, ...this.requires])) {
          ...
          if (path.parent.type === 'ReturnStatement') {
            extendClass.push(th.buildReturn({
    ...
    CallExpression(path) {
      ...
      if (this.superClassName) {
        ...
        if (t.isSuper(node.callee)) {
          // console.log(require('util').inspect(node, { depth: 2 }));
          replaceConstructorSuperCall(path, node, this.superClassName)
        }
        else if (node.callee.object && t.isSuper(node.callee.object)) {
  ...
  function replaceConstructorSuperCall(path, node, superClassName) {
    ...
    path.replaceWith(
      t.callExpression(t.identifier(`${superClassName}.prototype.constructor.apply`), [
        t.identifier('this'),
