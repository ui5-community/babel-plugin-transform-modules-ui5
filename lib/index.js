...
module.exports = ({ types: t }) => {
  ...
  const UI5ModuleVisitor = {
    ...
    Program: {
      ...
      enter(path, state) {
        ...
        path.state = {
          ui5: {
            wrap: null, // uninitialized defaults to true
            imports: [],
            export: null
          }
        }
      ...
      exit(path) {
        ...
        const defineCallExpression = generateDefine(state, path.node.body)
    ...
    ExportDeclaration(path) {
      ...
      if (path.node.type !== 'ExportDefaultDeclaration') {
        throw path.buildCodeFrameError("Only default exports are supported for ui5")
      }
      const state = path.state.ui5
      state.wrap = true
      ...
      if (declaration.type !== 'ClassDeclaration') {
        // For a standard export default, we'll save it to return at the the end.
        // This could include an object literal or a variable name.
        // TODO see if there is a way to export default a named const as it's declared
        state.export = path.node.declaration
        path.remove()
      }
      else {
        // For a class, we'll leave the declaration of the class where it is,
        // but saved it's name to return it from the module.
        const className = declaration.id.name
        state.export = t.identifier(className)
    ...
    ClassDeclaration: {
      enter(path) {
        if (this.ran) return
        const node = path.node
        const imports = path.state.ui5.imports
    ...
    CallExpression(path) {
      ...
      // If we encounter a function without a warpping iife or sap.ui.define, then we need to wrap with sap.ui.defined
      else if (state.wrap !== false) {
        state.wrap = true
      }
  ...
  function generateDefine(state, body) {
    const defineCallArgs = [
      ...
      t.functionExpression(
        ...
        t.blockStatement([
          ...body,
          t.returnStatement(state.export)
        ]
      ))
    ]
  ...
  function replaceClassWithUI5Extend(path) { // eslint-disable-line no-unused-vars
    ...
    const extendCall = t.callExpression(
      t.identifier(superClassName + ".extend"), extendCallArgs
    )
