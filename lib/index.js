    ...
    Class: {
      enter(path, { opts = {} }) {
        ...
        if (doesClassExtendFromImport(node, [...this.imports, ...this.requires])) {
          ...
          // TODO use parent block instead of programNode (not direct parent though, as it may be return statement)
          const staticProps = groupPropertiesByName(getOtherPropertiesOfIdentifier(this.programNode, name))
          const ui5ExtendClass = convertClassToUI5Extend(node, classInfo, staticProps)

          if (t.isClassDeclaration(node)) { // class X {}
            path.replaceWithMultiple(ui5ExtendClass)
          }
          else if (t.isClassExpression(node)) { // return class X {}
