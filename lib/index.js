...
module.exports = ({ types: t }) => {
  ...
  function convertClassToUI5Extend(node, classInfo) { // eslint-disable-line no-unused-vars
    ...
    //
    // }
    // console.log("constructor:", require('util').inspect(constructor, { depth: null }));
    // console.log(require('util').inspect(boundProps, { depth: 3 }));

    // Arrow function properties need to get moved to the constructor so that
    // they're bound properly to 'this' as the class instance. This is aligned with the class spec.
    ...
    const extendAssign = buildExtendAssign({
      NAME: t.identifier(className),
      SUPERNAME: t.identifier(superClassName),
