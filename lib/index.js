...
const buildConstructorProperty = template(`
      ...
      if (node.key.name === CONSTRUCTOR) {
        ...
        path.replaceWith(
          buildConstructorProperty({
            PARAMS: node.params,
            BODY: node.body.body // extra .body to remove the wrapping BlockStatement
          })
        )
      }
    }
  ...
  function generateDefine(state, body) {
    const defineCall = buildDefine({
      ...
      FACTORY: buildDefineCallback({
        PARAMS: state.imports.map(i => t.identifier(i.name)),
        BODY: [
          ...body,
