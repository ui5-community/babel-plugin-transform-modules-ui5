...
module.exports = ({ types: t }) => {
  ...
  const UI5ModuleVisitor = {
    ...
    ExportNamedDeclaration(path) {
      ...
      if (specifiers && specifiers.length) { // export { one, two }
        ...
        if (source) { // export { one, two } from 'x'
          ...
          const localName = src.replace(/\//g, '_').replace(/\./g, '')
          const _import = { src, name: localName }
          this.imports.push(_import)
          fromSource = tempModuleName(`${localName}.`)
        }
    ...
    ExportAllDeclaration(path) {
      ...
      this.imports.push({ src, name })
      // const  = tempModuleName(name)

      this.exportAllHelper = true

      this.exports.push({
        all: true,
        LOCAL: t.identifier(tempModuleName(name)),
      })
