...
export function convertClassToUI5Extend(node, classInfo, extraStaticProps) { // eslint-disable-line no-unused-vars
  ...
  // For controllers, we'll use onInit rather than constructor, since controller constructors don't work.

  // let { constructor, onInit, init } = propsByName

  const bindToMethodName = isController ? 'onInit' : 'constructor'
  const bindToId = t.identifier(bindToMethodName)
  // console.log("isController::", className, isController, bindToMethodName, bindToId);
  let bindMethod = isController ? propsByName[bindToMethodName] : constructor // avoid getting a prop named constructor as it may return {}'s
  const constructorJsdoc = getTags(constructorComments)
  const keepConstructor = classInfo.keepConstructor || constructorJsdoc.keep

  const needsBindingMethod = boundProps.length || (isController && constructor && !keepConstructor)
  ...
  if (isController && constructor) {
    if (keepConstructor) {
      ...
      bindMethod.body.body.push(
        ...(constructor.body.body
          .filter(node => {
            !(t.isCallExpression(node.expression) && node.expression.callee.type === 'Super')
          })
        )
      )
    }
