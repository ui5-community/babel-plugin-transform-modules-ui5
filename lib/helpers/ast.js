...
export function findPropertiesOfNode(programNode, declaration) {
  if (t.isFunctionDeclaration(declaration) || t.isArrowFunctionExpression(declaration)) {
    ...
    return [
      ...
      ...getOtherPropertiesOfIdentifier(programNode, declaration.id.name)
    ]
  }
  else if (t.isIdentifier(declaration)) {
    return getOtherPropertiesOfIdentifier(programNode, declaration.name)
  }
  else if (isObjectAssignOrExtendsExpression(declaration)) {
    return getPropertiesOfObjectAssignOrExtendHelper(declaration, programNode)
  }
  return null
}
 ...
 */
export function getOtherPropertiesOfIdentifier(programNode, idName) {
  return flatten(
    programNode.body
      .map(node => {
...
export function getPropertiesOfObjectAssignOrExtendHelper(node, programNode) {
  ...
  return flatten(
    node.arguments.map(arg => {
      if (t.isObjectExpression(arg)) {
        ...
        return getOtherPropertiesOfIdentifier(programNode, arg.name)
      }
    })
  )
}
