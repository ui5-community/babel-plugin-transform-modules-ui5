...
var th = _interopRequireWildcard(require("./helpers/templates"));

var ast = _interopRequireWildcard(require("./helpers/ast"));
...
module.exports = () => {
  const ProgramVisitor = {
    ...
    Program(path, {
      ...
      this.programNode = node; // Opts handling
  ...
  const UI5ModuleVisitor = {
    ...
    ImportDeclaration(path) {
      ...
      const shouldInterop = !this.noImportInteropPrefixesRegexp.test(src); // NOTE: DO NOT REMOVE. This code is for future enhancements, not old logic.
      ...
      for (const specifier of specifiers) {
        if (_core.types.isImportDefaultSpecifier(specifier)) {
          // import X from 'X'
          imp.default = true;
          imp.interop = shouldInterop; // Shorten the imported-as name since it should be unique for default imports.
      ...
      imp.destructors = imp.destructors.concat(destructors); // TODO: now that we're saving destructors on the imp, dynamically determine firstImport if needed.
    ...
    ExportDefaultDeclaration(path) {
      ...
      if (declarationName) {
        ...
        if (_core.types.isFunctionDeclaration(declaration)) {
          ...
          this.defaultExport = _core.types.functionExpression(null, params, body, generator, isAsync);
        } else {
    ...
    Class: {
      enter(path, {
        ...
        classInfo.name || classInfo.alias || /.*[.]controller[.]js$/.test(file.opts.filename) && opts.autoConvertControllerClass !== false; // default true
        ...
        this.superClassName = classInfo.superClassName; // Find the Block scoped parent (Program or Function body) and search for assigned properties within that.

        const blockParent = path.findParent(path => path.isBlock()).node;
        const staticProps = ast.groupPropertiesByName(ast.getOtherPropertiesOfIdentifier(blockParent, name)); // TODO: flag metadata and renderer for removal if applicable
    ...
    CallExpression(path) {
      ...
      if (ast.isImport(callee)) {
        ...
        path.replaceWith(_objectSpread({}, node, {
  ...
  function replaceSuperNamedCall(path, node, superClassName, methodName) {
    ...
    const callArgs = hasSpread ? [thisEx, _core.types.arrayExpression(node.arguments)] : [thisEx, ...node.arguments]; // console.log(callArgs)
